...
// renderização.
float g_CameraTheta = 0.50f;   // Ângulo no plano ZX em relação ao eixo Z
float g_CameraPhi = 0.50f;     // Ângulo em relação ao eixo Y
float g_CameraDistance = 2.5f; // Distância da câmera para a origem

float g_CamDistanceX = 1.50f;
float g_CamDistanceY = 2.5f;
float g_CamDistanceZ = 4.30f;
...
int main()
{
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    if (pressedW)
    {
      g_CamDistanceY -= (step * sin_g_CameraPhi);
      g_CamDistanceZ -= (step * cos_g_CameraPhi * cos_g_CameraTheta);
      g_CamDistanceX -= (step * cos_g_CameraPhi * sin_g_CameraTheta);
    }
    if (pressedS)
    {
      g_CamDistanceY += (step * sin_g_CameraPhi);
    ...
    if (pressedA)
    {
      g_CamDistanceX -= (step * cos_g_CameraPhi * cos_g_CameraTheta);
      g_CamDistanceZ += (step * cos_g_CameraPhi * sin_g_CameraTheta);
    }
    if (pressedD)
    {
      g_CamDistanceX += (step * cos_g_CameraPhi * cos_g_CameraTheta);
...
void KeyCallback(GLFWwindow *window, int key, int scancode, int action, int mod)
{
  ...
  // A W S D para movimentar a câmera | LAB 02
  if (key == GLFW_KEY_W)
