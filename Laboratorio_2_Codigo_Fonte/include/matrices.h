...
glm::mat4 Matrix_Rotate_Z(float angle)
{
    ...
    float s = sin(angle);
    return Matrix(
        // MATRIZ DE ROTAÇÃO (3D) EM TORNO DO EIXO Z EM COORD.
...
glm::mat4 Matrix_Rotate(float angle, glm::vec4 axis)
{
    ...
    return Matrix(
        // MATRIZ DE ROTAÇÃO (3D) EM TORNO DO EIXO v EM COORD. HOMOGÊNEAS,
        // UTILIZANDO OS PARÂMETROS vx, vy, vz, c e s (FÓRMULA DE RODRIGUES)
...
glm::mat4 Matrix_Perspective(float field_of_view, float aspect, float n, float f)
{
    ...
    glm::mat4 P = Matrix(
        // MATRIZ P DE PROJEÇÃO PERSPECTIVA (3D) UTILIZANDO OS PARÂMETROS n e f.
