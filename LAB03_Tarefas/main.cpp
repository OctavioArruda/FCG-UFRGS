...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 00246622 - Octavio Arruda", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
            ...
            PushMatrix(model); // Guardamos matriz model atual na pilha
                ...
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    ...
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        ...
                        // Right hand:
                        PushMatrix(model);
                            model *= Matrix_Translate(0.0f, -1.05f, 0.0f);
                            PushMatrix(model);
                                model *= Matrix_Scale(1.0f, 0.15f, 1.0f);
                                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                                DrawCube(render_as_black_uniform);  // Drawing right hand
                            PopMatrix(model);
                        PopMatrix(model);
                    PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
        ...
        // Left arm:
        PushMatrix(model);
            model *= Matrix_Translate(+0.55f, 0.0f, 0.0f);
            PushMatrix(model);
                model = model * Matrix_Rotate_Z(-g_AngleZ)
                              * Matrix_Rotate_Y(g_AngleY)
                              * Matrix_Rotate_X(g_AngleX);
                PushMatrix(model);
                    model *= Matrix_Scale(0.2f, 0.6f, 0.2f);
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                    DrawCube(render_as_black_uniform);
                PopMatrix(model);
                PushMatrix(model);
                    model *= Matrix_Translate(0.0f, -0.65f, 0.0f);
                    model = model * Matrix_Rotate_Z(-g_ForearmAngleZ)
        ...
        // Head:
        PushMatrix(model);
            model *= Matrix_Rotate_Z(3.14159); // pi rad rotation (180)
            ...
            PushMatrix(model);
                model = model * Matrix_Rotate_Z(g_AngleZ)
                              ...
                              * Matrix_Rotate_X(g_AngleX);
                PushMatrix(model);
                    model *= Matrix_Scale(0.3f, 0.3f, 0.3f);
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                    DrawCube(render_as_black_uniform);
                PopMatrix(model);
            PopMatrix(model);
        ...
        // Right leg:
        PushMatrix(model);
            model *= Matrix_Translate(-0.2f, -1.05f, 0.0f);
            PushMatrix(model);
                model *= Matrix_Scale(0.3f, 0.7f, 0.3f);
                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                DrawCube(render_as_black_uniform);
            PopMatrix(model);
            ...
            PushMatrix(model);
                ...
                PushMatrix(model);
                    model *= Matrix_Scale(0.25f, 0.7f, 0.25f);
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                    DrawCube(render_as_black_uniform);

                    // Right foot:
                    PushMatrix(model);
                        model *= Matrix_Translate(0.0f, -1.05f, 0.4f);
                        PushMatrix(model);
                            model *= Matrix_Scale(1.0f, 0.15f, 2.0f);
                            glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                            DrawCube(render_as_black_uniform);
                        PopMatrix(model);
                    PopMatrix(model);
